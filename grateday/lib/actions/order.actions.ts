...
export async function approvePayPalOrder(
    ...
    data: {orderID: string}) { 
      ...
      captureData.status !== 'COMPLETED') {
        throw new Error('Error in PayPal patment');
      }
    ...
    async function updateOrderToPaid({
      ...
      // Transaction to update order and account for product
      await prisma.$transaction(async (tx) => {
        // Iterate over products and uodate stock
      ...
      // Get updated order after trancation
      const uodateOrder = await prisma.order.findFirst({
        where: {id: orderId},
        include: {
            orderitems: true,
            user: { select: {name: true, email: true} },
      ...
      if(!uodateOrder) throw new Error('Order not found');
    }
